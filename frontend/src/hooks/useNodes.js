  ...
  const addNodeFromLine = useCallback((parentId, childIdsOrTargetChildId, options = {}) => {
    ...
    setProjects(prev => {
      ...
      if (isHorizontalConnectorButton) {
        ...
        // =====================================================
        // APLICAR ALINEACIÓN RECURSIVA A LOS SUBÁRBOLES
        // Esto asegura que si los hijos del conector tienen
        // sus propios hijos, estos también se redistribuyan
        // =====================================================
        
        // Función local para alinear recursivamente
        const alignConnectorSubtrees = (nodes) => {
          const margin = 60;
          const minSpacing = 180;
          const cWidth = 160;
          
          // Función para calcular ancho de subárbol
          const getWidth = (nodeId, allNodes) => {
            const node = allNodes.find(n => n.id === nodeId);
            if (!node) return cWidth;
            
            const vertKids = allNodes.filter(n => 
              n.parentId === nodeId && n.childDirection === 'vertical'
            );
            const horizKids = allNodes.filter(n => 
              n.parentId === nodeId && n.childDirection === 'horizontal'
            );
            
            let vertWidth = cWidth;
            if (vertKids.length > 0) {
            ...
            let horizMaxWidth = 0;
            horizKids.forEach(child => {
              horizMaxWidth = Math.max(horizMaxWidth, getWidth(child.id, allNodes));
            });
            
            return Math.max(cWidth, vertWidth, horizMaxWidth);
          };
          
          // Función recursiva para alinear
          const alignNode = (nodeId, currentNodes) => {
            const node = currentNodes.find(n => n.id === nodeId);
            if (!node) return currentNodes;
            
            let aligned = [...currentNodes];
            const pWidth = node.width || 160;
            const pHeight = node.height || 64;
            const vGap = 100;
            const hGap = 200;
            ...
            hChildren.forEach((child) => {
              aligned = alignNode(child.id, aligned);
            });
            
            // Procesar hijos verticales
            const vChildren = aligned.filter(n => 
              n.parentId === nodeId && n.childDirection === 'vertical'
            ).sort((a, b) => a.x - b.x);
          ...
          // Alinear todos los nodos del conector y sus subárboles
          let aligned = [...nodes];
          const connectorNodes = aligned.filter(n => 
            n.connectorParentId === parentId && n.connectorTargetId === childIdsOrTargetChildId
          );
          
          connectorNodes.forEach(connNode => {
            aligned = alignNode(connNode.id, aligned);
          });
          
          return aligned;
        };
        
        // Aplicar alineación a subárboles
        newNodes = alignConnectorSubtrees(newNodes);
